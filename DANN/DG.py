...
def train_dann(model, source_loader, target_loader, num_epochs, lr, alpha):
    ...
    for epoch in range(num_epochs):
        source_label_counter = 0
        for (source_data, source_labels), (target_data, _) in zip(source_loader, target_loader):
            source_data, source_labels, target_data = source_data.to(device), source_labels.to(device), target_data.to(device)

            l = (source_label_counter // 2526) + 1
            source_domain_labels = torch.ones(source_data.shape[0], 1).to(device) * l
            source_label_counter += source_loader.batch_size

            class_logits, domain_logits = model(source_data, alpha)

            class_loss = criterion_class(class_logits[:source_data.shape[0]], source_labels)
            domain_loss = criterion_domain(domain_logits, source_domain_labels)
