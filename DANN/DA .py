...
# 训练函数
def train_dann(model, source_loader, target_loader, num_epochs, lr, alpha):
    ...
    for epoch in range(num_epochs):
        for (source_data, source_labels), (target_data, _) in zip(source_loader, target_loader):
            ...
            source_domain_labels = torch.ones(source_data.shape[0], 1).to(device)
            target_domain_labels = torch.zeros(target_data.shape[0], 1).to(device)
            ...
            class_loss = criterion_class(class_logits[:source_data.shape[0]], source_labels)
            domain_loss = criterion_domain(domain_logits, combined_domain_labels)
...
for test_subject in tqdm(range(1, 13), desc="Testing Subjects"):
    ...
    ccombined_data = np.concatenate((train_data, test_data), axis=0)
    ...
    ccombined_dataset = ConcatDataset([source_dataset, target_dataset])

    source_loader = DataLoader(source_dataset, batch_size=32, shuffle=True)
    target_loader = DataLoader(target_dataset, batch_size=32, shuffle=True)
    ccombined_loader = DataLoader(ccombined_dataset, batch_size=32, shuffle=True)
    ...
    train_dann(dann_model, source_loader, target_loader, num_epochs, lr, alpha)
   ...
   # t-SNE降维可视化
    if test_subject == 1:
        ...
        with torch.no_grad():
            slabel_counter = 0
            for data, labels in ccombined_loader:
                data = data.to(device)
                features = dann_model.feature_extractor(data)
                dann_features.append(features.cpu().numpy())

                l = (slabel_counter // 27786) + 1
        ...
        train_data_tsne = tsne.fit_transform(ccombined_data)
